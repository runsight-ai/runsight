...
test.describe("Tasks CRUD", () => {
  ...
  test("create task", async ({ page }) => {
    ...
    await modal.getByPlaceholder(/Enter task name/i).fill(testTaskName);
    await modal.getByPlaceholder(/Describe what this task does/i).fill("E2E test task description");
    await modal.getByRole("button", { name: /Create/i }).click();

    await expect(modal).not.toBeVisible({ timeout: 10000 });
    await expect(page.getByText(testTaskName, { exact: true })).toBeVisible({ timeout: 10000 });

    const afterData = await apiGet("/tasks");
    const created = afterData.items.find((t: { name: string }) => t.name === testTaskName);
  ...
  test("edit task", async ({ page }) => {
    test.skip(!createdTaskId, "Task was not created in previous test");
