...
const harness = vi.hoisted(() => {
  const canvasState = {
    ...
    isDirty: false,
    yamlContent: "id: wf_rerun_current\nkind: workflow\nversion: '1.0'\n",
    setNodeStatus: vi.fn(),
...
beforeEach(() => {
  ...
  harness.createRunRequest.mockResolvedValue({ id: "run_rerun_new" });
  harness.prepareSimulation.mockReset();
  harness.prepareSimulation.mockResolvedValue({
    branch: "sim/wf_rerun_current/20260422/abc12",
...
describe("rerun workflow inputs from footer history", () => {
  ...
  it("skips the rerun modal when the dirty simulation snapshot no longer has inputs", async () => {
    ...
    harness.prepareSimulation.mockResolvedValueOnce({
      branch: "sim/wf_rerun_current/20260422/xyz99",
