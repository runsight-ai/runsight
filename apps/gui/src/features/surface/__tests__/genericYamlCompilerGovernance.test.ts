/**
 * Governance: generic YAML parser/compiler boundary.
 *
 * Owner: GUI Surface YAML parser/compiler.
 * Boundary: compiler and parser behavior stays generic and does not re-add
 * hardcoded block field maps or known-block-type allowlists.
 * Exit criteria: remove once block schema metadata is generated from a shared
 * registry instead of guarded by source checks.
 *
 * The frontend compiler and parser use ONLY the generic
...
describe("Legacy cleanup: yamlParser.ts", () => {
  ...
  it("does not contain VALID_STEP_TYPES constant (legacy name)", () => {
...
describe("Round-trip after cleanup: known types use generic path", () => {
  ...
  it("http_request block round-trips all fields", () => {
    ...
    expect(getBlock(doc1, "b1").url).toBe("https://api.example.test");
...
describe("Mixed known + unknown types after cleanup", () => {
  it("workflow with linear, custom_thing, and http_request all round-trip", () => {
    ...
    expect(getBlock(doc1, "fetch").url).toBe("https://api.example.test/data");
...
describe("Parse from YAML: known types via generic path", () => {
  ...
  it("http_request block parses all snake_case fields to camelCase", () => {
    ...
    expect(data.url).toBe("https://api.example.test");
...
// ===========================================================================
// 6. Editor type surface contract
// ===========================================================================
