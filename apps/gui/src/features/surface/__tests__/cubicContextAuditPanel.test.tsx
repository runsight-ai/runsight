...
function makeEvents(runId: string, prefix: string, start: number, count: number) {
  return Array.from({ length: count }, (_, offset) => {
    ...
    const label = index < 100 ? `visible-${index}` : `extra-${index - 100}`;
    return contextEvent(runId, `${prefix}-${label}`, index + 1);
  });
}

describe("context audit pagination regressions", () => {
  beforeEach(() => {
  ...
  it("resets visible rows when the selected run changes", async () => {
    ...
    expect(screen.getByText("node-a-extra-49")).toBeTruthy();
    ...
    await waitFor(() => {
      expect(screen.queryByText("node-b-extra-49")).toBeNull();
    });
    expect(screen.getByText("node-b-visible-99")).toBeTruthy();
  });
  ...
  it("shows rows fetched from the next remote page immediately", async () => {
    ...
    expect(screen.queryByText("node-c-extra-49")).toBeNull();
    await user.click(screen.getByRole("button", { name: "Load more" }));

    await waitFor(() => {
      expect(screen.getByText("node-c-extra-49")).toBeTruthy();
    });
