...
beforeEach(() => {
  ...
  harness.inModalSubmit = false;
  harness.nextRunId = "run-904-success";
});
...
describe("RUN-904 RunButton input modal wiring", () => {
  it("runs immediately when the resolved workflow has no inputs", async () => {
    ...
    renderRunButton({ workflowId: "wf_run_904_clean", isCommitted: true });
    fireEvent.click(screen.getByRole("button", { name: /run/i }));

    await waitFor(() => {
      expect(harness.createRunMutate).toHaveBeenCalledWith(
        {
          workflow_id: "wf_run_904_clean",
          inputs: {},
          source: "manual",
          branch: "main",
        },
    ...
    expect(harness.state.setActiveRunId).toHaveBeenCalledWith("run-904-success");
  });
  ...
  it("opens the run inputs modal and waits for submit when the workflow has inputs", async () => {
    harness.workflowById.wf_run_904_inputs = {
      id: "wf_run_904_inputs",
      input_schema: inputSchema,
    };

    renderRunButton({ workflowId: "wf_run_904_inputs", isCommitted: true });
    fireEvent.click(screen.getByRole("button", { name: /run/i }));
