...
vi.mock("../../surface/useForkWorkflow", () => ({
  useForkWorkflow: (options: {
    ...
    harness.forkWorkflow.mockImplementation(() => {
      ...
      options.onTransition?.("wf_forked_topbar");
    });
...
function setReadonlyFixtures({
  ...
  harness.run = {
    id: "run_readonly_topbar",
    workflow_id: "wf_readonly_topbar",
    workflow_name: "Research Pipeline",
    status: runStatus,
    commit_sha: "commit_topbar",
    duration_seconds: 75,
    total_tokens: 1234,
    total_cost_usd: 4.2,
    source: "manual",
  ...
  harness.workflow = {
    id: "wf_readonly_topbar",
    name: "Research Pipeline",
    yaml: "workflow:\n  name: Research Pipeline\n",
    canvas_state: null,
    commit_sha: "workflow_topbar",
  };
  harness.runNodes = [];
}
...
describe("WorkflowSurface readonly topbar wiring", () => {
  it("shows completed readonly run metadata, review badge, and fork action", async () => {
    ...
    render(
      <MemoryRouter>
        <WorkflowSurface mode="readonly" runId="run_readonly_topbar" workflowId="wf_readonly_topbar" />
      </MemoryRouter>,
    );
    ...
    expect(harness.forkInvocations[0]).toEqual({
      commitSha: "commit_topbar",
      workflowPath: "custom/workflows/wf_readonly_topbar.yaml",
    ...
    expect(screen.getByRole("link", { name: "Research Pipeline" }).getAttribute("href")).toBe(
      "/workflows/wf_readonly_topbar/edit",
    );
