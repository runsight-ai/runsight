...
interface EditableWorkflowSurfaceRouteProps {
  workflowId: string;
  initialMode: WorkflowSurfaceMode;
}

export function EditableWorkflowSurfaceRoute({
  workflowId,
  initialMode,
}: EditableWorkflowSurfaceRouteProps) {
  ...
  const surfaceMode = initialMode === "fork-draft" ? "fork-draft" : "workflow";
  ...
  }, [activeProviders.length, createRun, handleOpenApiKeyModal, workflowId, isCommitted, isDirty, setActiveRunId]);
  ...
  const currentFiles = [{ path: `custom/workflows/${workflowId}.yaml`, status: "modified" }];
  ...
  return (
    <>
      <SurfaceComponent
        ...
        footer={<CanvasBottomPanel workflowId={workflowId} />}
...
export const Component = EditableWorkflowSurfaceRoute;

export default EditableWorkflowSurfaceRoute;
