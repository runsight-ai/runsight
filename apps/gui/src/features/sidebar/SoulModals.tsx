...
export function NewSoulModal({ open, onClose }: NewSoulModalProps) {
  ...
  const isSystemPromptFilled = systemPrompt.trim().length > 0;
  ...
  const handleSubmit = async () => {
    if (!isNameFilled || !isSystemPromptFilled || isSubmitting) return;
    ...
    try {
      await createSoul.mutateAsync({
        ...
        model_name: selectedModels.length > 0 ? selectedModels[0] : null,
      });
      onClose();
    } catch (error) {
  ...
  const toggleModel = (model: string) => {
    setSelectedModels((prev) => (prev[0] === model ? [] : [model]));
  };
    ...
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleCancel()}>
      <DialogContent className="w-[560px] max-w-[560px] p-0 gap-0 bg-[var(--surface-secondary)] border-[var(--border-default)] rounded-xl overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted">
              Role <span className="text-[var(--danger-9)]">*</span>
            </Label>
            <Input
              type="text"
              placeholder="Enter soul role..."
              value={name}
            ...
            />
            <p className="text-xs text-[var(--text-muted)]">The role this soul will play in workflows</p>
          </div>
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted">
              System Prompt <span className="text-[var(--danger-9)]">*</span>
            </Label>
            <Textarea
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-[var(--text-muted)]">Choose the default model for this soul</p>
          </div>
        </div>
        <DialogFooter className="h-16 px-4 border-t border-[var(--border-default)] flex items-center justify-end gap-2 shrink-0">
          ...
          <Button onClick={handleSubmit} disabled={!isNameFilled || !isSystemPromptFilled || isSubmitting} className="h-9 px-4 bg-[var(--interactive-default)] hover:bg-[var(--interactive-hover)] text-on-accent disabled:opacity-40">
            {isSubmitting ? (
              <>
      ...
      <DialogContent className="w-[560px] max-w-[560px] p-0 gap-0 bg-[var(--surface-secondary)] border-[var(--border-default)] rounded-xl overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <div className="space-y-2">
            ...
            <Input type="text" placeholder="Enter soul role..." value={name} onChange={(e) => setName(e.target.value)} className="h-9 bg-[var(--surface-secondary)] border-[var(--border-default)] rounded-md text-sm text-primary placeholder:text-[var(--text-muted)] focus:border-[var(--interactive-default)] focus:ring-0 focus-visible:ring-0 focus-visible:ring-offset-0" disabled={isSubmitting} />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <Label className="text-xs font-semibold uppercase tracking-wider text-muted">Model</Label>
            <div className="flex flex-wrap gap-2">
              {AVAILABLE_MODELS.map((model) => (
