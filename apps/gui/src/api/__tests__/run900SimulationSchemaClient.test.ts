...
describe("RUN-900 simulation snapshot client contract", () => {
  it("createSimBranch returns the prepared input_schema alongside branch and commit_sha", async () => {
    ...
    const response = (await gitApi.createSimBranch("wf_900", yamlContent)) as {
      branch: string;
      commit_sha: string;
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/workflows/wf_900/simulations",
  ...
  it("createSimBranch rejects a simulation response with an unknown input type instead of guessing", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({
        branch: "sim/wf_900/20260419/def34",
    ...
    await expect(
      gitApi.createSimBranch("wf_900", "workflow:\n  name: Broken Flow\n"),
    ).rejects.toThrow();
  });
});
