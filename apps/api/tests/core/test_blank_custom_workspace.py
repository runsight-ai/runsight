"""Blank workspace startup and governance coverage."""
...
class TestBlankWorkspaceSourceStartup:
    def test_source_startup_creates_empty_custom_dirs_and_no_sample_yaml(
...
class TestCustomAssetGovernance:
    """Governance: protect repo-root custom/ as runtime data, not shipped fixtures."""

    def test_repo_root_custom_tree_does_not_ship_sample_yaml_assets(self):
        """Exit when release packaging cannot include sample custom YAML assets."""
...
class TestExistingUserAssetsPreserved:
    def test_existing_custom_assets_remain_unchanged(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ):
