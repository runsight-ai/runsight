"""ExecutionService avoids duplicate observer lifecycle calls.

Workflow.run() is the single source of observer lifecycle events. These tests
protect against duplicate LogEntry records and duplicate Run status updates.
"""
...
class TestOnWorkflowStartCalledOnce:
    ...
    async def test_on_workflow_start_called_once_not_twice(self):
        ...
        Workflow.run() owns the lifecycle callback, so _run_workflow() must not
        call the observer manually as well.
        """
        svc = _make_service()
...
class TestOnWorkflowCompleteCalledOnce:
    ...
    async def test_on_workflow_complete_called_once_not_twice(self):
        ...
        Workflow.run() owns the terminal callback, so _run_workflow() must not
        call the observer manually as well.
        """
        svc = _make_service()
...
class TestOnWorkflowErrorCalledOnce:
    ...
    async def test_on_workflow_error_called_once_not_twice(self):
        ...
        Workflow.run() owns the error callback before re-raising, so
        _run_workflow() must not emit the same lifecycle event again.
        """
        svc = _make_service()
