...
class TestLateStreamSubscribers:
    ...
    @pytest.mark.asyncio
    async def test_all_attached_subscribers_terminate_when_queued_run_is_cancelled_before_start(
        self,
    ):
        """Every attached subscriber must terminate on queued pre-start cancellation.

        Early observer registration is only safe if cancellation closes the
        stream for all already-attached subscribers, not just whichever
        subscriber happens to consume the queue's single close sentinel first.
        """
        ...
        run_id = "run_952_stream_cancelled_queued_multi"
        ...
        first_consumer = asyncio.create_task(consume(first_events))
        second_consumer = asyncio.create_task(consume(second_events))

        await asyncio.sleep(0.05)
        assert not first_consumer.done()
        assert not second_consumer.done()
        ...
        assert first_consumer.done() and second_consumer.done(), (
            "Queued pre-start cancellation should terminate every attached subscriber, "
            "not leave one hanging on an emptied shared queue."
        )
        assert first_events == [] or first_events[-1]["event"] in {
            "run_cancelled",
            "run_failed",
            "run_completed",
        }
        assert second_events == [] or second_events[-1]["event"] in {
            "run_cancelled",
            "run_failed",
            "run_completed",
        }
