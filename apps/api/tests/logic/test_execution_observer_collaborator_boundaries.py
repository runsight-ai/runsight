"""Execution observer collaborator boundary coverage.
...
def _seed_run(
    engine,
    *,
    run_id: str = "run_observer_boundary",
    ...
    with Session(engine) as session:
        session.add(
            Run(
                id=run_id,
                workflow_id="wf_observer_boundary",
...
def _context_audit_event(node_id: str = "call_child") -> ContextAuditEventV1:
    return ContextAuditEventV1(
        run_id="run_observer_boundary",
        workflow_name="wf_observer_boundary",
        node_id=node_id,
        block_type="workflow",
        access="declared",
        mode="strict",
...
def test_workflow_start_run_write_failure_still_persists_workflow_start_log() -> None:
    ...
    with patch(
        ...
        observer.on_workflow_start("wf_observer_boundary", WorkflowState())
...
def test_block_complete_node_write_failure_still_persists_logs_and_trace_tail() -> None:
    ...
    with patch(
        ...
        observer.on_block_complete("wf_observer_boundary", "call_child", "workflow", 1.5, state)
...
def test_workflow_complete_run_write_failure_still_persists_logs_and_trace_tail() -> None:
    ...
    with patch(
        ...
        observer.on_workflow_complete("wf_observer_boundary", state, 8.5)
...
def test_workflow_error_run_write_failure_still_persists_terminal_log(
    ...
    with patch(
        ...
        observer.on_workflow_error("wf_observer_boundary", error, 9.0)
