...
def test_set_workflow_enabled_can_disable_with_targeted_repo_helper():
    workflow_repo = Mock()
    run_repo = Mock()
    workflow_repo.set_enabled.return_value = WorkflowEntity(
        id="wf_research",
    ...
    result = service.set_workflow_enabled("wf_research", False)

    assert result.enabled is False
    workflow_repo.set_enabled.assert_called_once_with("wf_research", False)
    workflow_repo.update.assert_not_called()
