...
class TestSharedSSRFUtilityExists:
    ...
    def test_shared_validator_blocks_private_ip(self):
        ...
        with pytest.raises(SSRFError):
            validate_ssrf("http://192.168.1.1/models")

    def test_shared_validator_allows_public_url(self):
        """The shared validator should not raise for public URLs."""
        from runsight_core.security import validate_ssrf

        # Should not raise
        validate_ssrf("https://api.openai.com/v1/models")

    def test_shared_validator_respects_allow_private_flag(self):
        """The shared validator should accept an allow_private flag."""
        from runsight_core.security import validate_ssrf

        # Should not raise when private IPs are explicitly allowed
        validate_ssrf("http://127.0.0.1/models", allow_private=True)
...
class TestSSRFBlocksPrivateIPs:
    ...
    def test_blocks_private_ip_192_168(self):
        ...
        result = service.test_connection("prov_test123")
...
class TestOllamaLocalhostException:
    ...
    def test_ollama_127_0_0_1_allowed(self):
    ...
    def test_ollama_default_url_allowed(self):
