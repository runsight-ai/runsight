"""API sensitive-input redaction boundaries."""
...
def _service(yaml: str = "") -> ExecutionService:
    ...
    workflow_repo._get_path.return_value = "/custom/workflows/sensitive_inputs_workflow.yaml"
...
def _seed_run(engine, run_id: str = "run_sensitive_api") -> None:
...
def _log_messages(engine, run_id: str = "run_sensitive_api") -> list[str]:
    with Session(engine) as session:
        logs = list(session.exec(select(LogEntry).where(LogEntry.run_id == run_id)).all())
    return [entry.message for entry in logs]


def _seed_eval_run(engine, run_id: str = "run_sensitive_eval", block_id: str = "block_a") -> None:
    with Session(engine) as session:
        session.add(
            Run(
                id=run_id,
