"""StreamingObserver wiring in the execution pipeline.
...
class TestSubscribeStreamYieldsEvents:
    ...
    @pytest.mark.asyncio
    async def test_subscribe_stream_uses_observer_registered_during_run(self):
        """subscribe_stream has a live observer to consume during execution."""
        svc = _make_service()
        ...
        # After the run, try subscribe_stream. A live observer should have
        # been available during execution, but may already be cleaned up here.
        events = []
        async for event in svc.subscribe_stream(run_id):
            events.append(event)

        # Observer cleanup after terminal completion is allowed. Verify the
        ...
        # Manual registration proves the stream contract; the second run below
        # verifies _run_workflow registers the observer during execution.
        assert len(manual_events) == 1

        # Run again and capture registration during execution.
        svc2 = _make_service()
...
class TestChildStreamingObserverIsolation:
    ...
    def test_clone_for_child_run_creates_a_distinct_queue(self) -> None:
        parent = StreamingObserver(run_id="run_streaming_parent")

        child = parent.clone_for_child_run(child_run_id="run_streaming_child")

        assert child.run_id == "run_streaming_child"
        assert child.parent_run_id == "run_streaming_parent"
    ...
    def test_sibling_child_observers_do_not_share_each_others_live_events(self) -> None:
        parent = StreamingObserver(run_id="run_streaming_parent")
        first_child = parent.clone_for_child_run(child_run_id="run_streaming_child_a")
        second_child = parent.clone_for_child_run(child_run_id="run_streaming_child_b")
    ...
    def test_child_queue_is_removed_after_child_completion(self) -> None:
        parent = StreamingObserver(run_id="run_streaming_parent")
        child = parent.clone_for_child_run(child_run_id="run_streaming_child_done")

        assert "run_streaming_child_done" in parent._child_queues
        ...
        assert "run_streaming_child_done" not in parent._child_queues, (
    ...
    def test_child_queue_is_removed_after_child_failure(self) -> None:
        parent = StreamingObserver(run_id="run_streaming_parent")
        child = parent.clone_for_child_run(child_run_id="run_streaming_child_failed")

        assert "run_streaming_child_failed" in parent._child_queues

        child.on_workflow_error("child_wf", RuntimeError("boom"), 0.25)
        assert child.is_done is True

        assert "run_streaming_child_failed" not in parent._child_queues, (
            "Parent StreamingObserver must release a child queue after that child stream "
