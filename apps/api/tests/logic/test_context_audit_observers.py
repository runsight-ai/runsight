"""
API observer coverage for context audit persistence and streaming.
...
def _seed_run(engine, run_id: str = "run_context_audit") -> None:
    with Session(engine) as session:
        session.add(
            Run(
                id=run_id,
                workflow_id="wf_context_audit",
...
def _logged_context_events(engine, run_id: str = "run_context_audit") -> list[LogEntry]:
    with Session(engine) as session:
        return list(
            session.exec(
                select(LogEntry)
...
def test_execution_observer_persists_context_audit_event_as_valid_json() -> None:
    ...
    engine = _db_engine()
    _seed_run(engine)
    observer = ExecutionObserver(engine=engine, run_id="run_context_audit")
...
def test_streaming_observer_queues_context_resolution_sse_event() -> None:
    ...
    observer = StreamingObserver(run_id="run_context_audit")
