"""Cancel endpoint wiring to asyncio.Task.cancel().

These tests verify ExecutionService.cancel_execution() task cancellation and
route-level integration with RunService.cancel_run().
"""
...
class TestCancelRouteIntegration:
    ...
    def _make_mock_run(self, run_id="run_cancel_route", status="cancelled"):
    ...
    def test_cancel_route_calls_execution_service(self):
        ...
        try:
            ...
            response = client.post("/api/runs/run_cancel_route/cancel")

            assert response.status_code == 200
            mock_exec_svc.cancel_execution.assert_called_once_with("run_cancel_route")
        finally:
    ...
    def test_cancel_route_still_calls_run_service(self):
        ...
        try:
            client = TestClient(app)
            client.post("/api/runs/run_cancel_route/cancel")

            mock_run_svc.cancel_run.assert_called_once_with("run_cancel_route")
        finally:
            app.dependency_overrides.clear()
