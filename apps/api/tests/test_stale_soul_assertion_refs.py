"""Verify stale soul-assertion references are removed from test files.

Four test files use `assertions` as a soul field to test unknown-field preservation.
The `assertions` field is being retired from SoulEntity, so these tests
...
# this file lives at apps/api/tests/test_stale_soul_assertion_refs.py
...
TARGET_FILES = {
    ...
    "api_soul_schema_alignment": (
        _REPO_ROOT / "apps" / "api" / "tests" / "domain" / "test_soul_schema_alignment.py"
    ),
    "api_soul_update_preservation": (
        _REPO_ROOT / "apps" / "api" / "tests" / "logic" / "test_soul_update_preservation.py"
    ),
