"""Child lifecycle SSE and shared transport contracts.
...
Contracts:
- RunResponse carries ``parent_run_id``, ``root_run_id``, and ``depth``.
- RunNodeResponse carries ``child_run_id`` and ``exit_handle``.
- StreamingObserver treats only root completion as terminal.
"""
...
_RUN_RESPONSE_BASE = dict(
    id="run_sse_child",
    workflow_id="wf_sse_child_lifecycle",
    workflow_name="SSE child lifecycle workflow",
    status="running",
    started_at=100.0,
    completed_at=None,
...
_NODE_RESPONSE_BASE = dict(
    id="run_sse_child:step_child_link",
    run_id="run_sse_child",
    node_id="step_child_link",
    block_type="llm",
    status="completed",
    started_at=100.0,
    completed_at=200.0,
...
class TestNoLegacyTerminalChildPath:
    ...
    def test_sse_endpoint_uses_terminal_events_constant(self):
        ...
        # Terminal handling must remain centralized on the shared constant.
