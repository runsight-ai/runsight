"""Canonical /api/runs route behavior."""
...
def _make_mock_run(run_id: str = "run_canonical_route"):
    mock_run = Mock()
    mock_run.id = run_id
    mock_run.workflow_id = "wf_canonical_runs"
    mock_run.workflow_name = "Canonical runs workflow"
...
def test_resolve_summaries_rejects_invalid_batch_shape_without_per_run_fallback():
    """Non-dict batch summaries are rejected instead of degrading to per-run reads."""
    mock_service = Mock()
    ...
    with pytest.raises(TypeError):
        runs_router._resolve_summaries(mock_service, ["run_canonical_route"], raw_batch=[])
...
def test_runs_route_does_not_fall_back_to_per_run_summaries_when_batch_shape_is_invalid():
    """GET /api/runs rejects invalid batch data without calling get_node_summary."""
    run = _make_mock_run()
