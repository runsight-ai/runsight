"""Historical context audit endpoint contracts."""
...
def _make_run(run_id: str = "run_context_audit_history"):
    run = Mock()
    run.id = run_id
    run.workflow_id = "wf_context_audit_endpoint"
...
def _audit_payload(
    *,
    ...
    return {
        ...
        "run_id": "run_context_audit_history",
...
def _log(
    ...
    *,
    ...
    log = Mock()
    log.id = log_id
    log.run_id = "run_context_audit_history"
    log.node_id = node_id
    log.level = "trace"
...
def test_context_audit_endpoint_returns_paginated_valid_audit_events_only() -> None:
    ...
    try:
        response = client.get("/api/runs/run_context_audit_history/context-audit?page_size=1")
    finally:
...
def test_context_audit_endpoint_filters_by_node_id() -> None:
    ...
    try:
        response = client.get("/api/runs/run_context_audit_history/context-audit?node_id=review")
    finally:
        app.dependency_overrides.clear()
...
def test_context_audit_endpoint_rejects_invalid_page_size() -> None:
    ...
    try:
        too_small = client.get("/api/runs/run_context_audit_history/context-audit?page_size=0")
        too_large = client.get("/api/runs/run_context_audit_history/context-audit?page_size=501")
    finally:
        app.dependency_overrides.clear()
...
def test_context_audit_endpoint_safe_error_does_not_expose_stack_trace() -> None:
    ...
    try:
        response = client.get("/api/runs/run_context_audit_history/context-audit")
    finally:
        app.dependency_overrides.clear()
