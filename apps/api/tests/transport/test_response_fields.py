...
def _make_mock_run(
    run_id: str = "run_response_list",
    *,
    branch: str = "feat/response-fields",
    source: str = "webhook",
    ...
    mock_run = Mock()
    mock_run.id = run_id
    mock_run.workflow_id = "wf_response_fields"
    mock_run.workflow_name = "Response fields workflow"
...
def test_runs_list_includes_branch_source_and_commit_sha():
    ...
    try:
        ...
        assert item["branch"] == "feat/response-fields"
...
def test_runs_get_includes_branch_source_and_commit_sha():
    mock_run = _make_mock_run(run_id="run_response_detail")
    mock_service = _stub_run_service(mock_run)
    app.dependency_overrides[get_run_service] = lambda: mock_service
    app.dependency_overrides[get_eval_service] = lambda: _mock_eval_svc()

    try:
        response = client.get("/api/runs/run_response_detail")
        assert response.status_code == 200
        body = response.json()
        assert body["branch"] == "feat/response-fields"
...
def test_runs_get_serializes_commit_sha_as_null_when_unset():
    mock_run = _make_mock_run(
        run_id="run_response_null_branch",
        branch="main",
        source="manual",
        commit_sha=None,
    )
    ...
    try:
        response = client.get("/api/runs/run_response_null_branch")
        assert response.status_code == 200
        body = response.json()
...
def test_runs_get_does_not_use_legacy_workflow_commit_sha_for_old_records():
    mock_run = _make_mock_run(
        run_id="run_response_legacy",
        branch="main",
        source="manual",
        commit_sha=None,
        workflow_commit_sha="legacysharesponse",
    )
    mock_service = _stub_run_service(mock_run)
    app.dependency_overrides[get_run_service] = lambda: mock_service
    app.dependency_overrides[get_eval_service] = lambda: _mock_eval_svc()
    ...
    try:
        response = client.get("/api/runs/run_response_legacy")
        assert response.status_code == 200
        body = response.json()
        assert body["commit_sha"] is None
