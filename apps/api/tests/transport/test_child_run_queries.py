...
Contracts:
- ``GET /runs/{run_id}/children`` returns child runs as RunResponse objects.
- ``GET /runs/{run_id}`` includes parent_run_id, root_run_id, and depth.
- ``GET /runs/{run_id}/nodes`` includes child_run_id on workflow-call nodes.
- The router delegates child lookups through ``RunService.list_children``.
"""
