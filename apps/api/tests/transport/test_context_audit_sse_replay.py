"""Normalized context audit SSE replay contracts."""
...
def _make_run(run_id: str = "run_context_audit_sse"):
    run = Mock()
    run.id = run_id
    run.workflow_id = "wf_context_audit_sse"
...
def _log(message: dict, log_id: int):
    log = Mock()
    log.id = log_id
    log.run_id = "run_context_audit_sse"
...
def test_sse_replay_emits_context_resolution_for_audit_logs_and_keeps_generic_replay() -> None:
    ...
    try:
        with client.stream("GET", "/api/runs/run_context_audit_sse/stream") as response:
