"""
task_data to inputs rename and tasks CRUD removal.

These tests verify the post-removal state.
"""
...
# ---------------------------------------------------------------------------
# 7. Governance/migration: docs execution guide uses `inputs`, not `task_data`
#
# Boundary: API task_data removal must not leave user-facing docs advertising
# the retired request field.
# Owner: apps/site docs workspace.
# Exit criteria: move this check to the docs workspace once docs tests own
# execution-guide request examples.
# ---------------------------------------------------------------------------


def test_running_workflows_md_has_no_task_data():
    """Governance: running-workflows.md must use `inputs` instead of `task_data`."""
    docs_path = (
...
def test_running_workflows_md_mentions_inputs():
    """Governance: running-workflows.md must reference `inputs` after the rename."""
    docs_path = (
        Path(__file__).parents[4]
        / "apps"
        / "site"
