"""Context audit API and SSE contract integration."""
...
def _seed_run(engine, run_id: str = "run_context_audit_contract") -> None:
    with Session(engine) as session:
        session.add(
            Run(
                id=run_id,
                workflow_id="wf_context_audit_contract",
...
def _event(run_id: str = "run_context_audit_contract") -> ContextAuditEventV1:
...
def _mock_run(run_id: str = "run_context_audit_contract"):
    run = Mock()
    run.id = run_id
    run.workflow_id = "wf_context_audit_contract"
...
def _malformed_log():
    log = Mock()
    log.id = "malformed"
    log.run_id = "run_context_audit_contract"
    log.node_id = "broken"
    log.level = "trace"
...
def test_execution_streaming_history_and_gui_contract_payloads_match() -> None:
    ...
    _seed_run(engine)
    event = _event()
    execution_observer = ExecutionObserver(engine=engine, run_id="run_context_audit_contract")
    streaming_observer = StreamingObserver(run_id="run_context_audit_contract")
    ...
    try:
        response = client.get("/api/runs/run_context_audit_contract/context-audit")
    finally:
