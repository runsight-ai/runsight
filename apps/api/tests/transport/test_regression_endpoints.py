"""Per-run regression count and issue list.

Tests cover:
- RunResponse includes regression_count field
- GET /api/runs/:id/regressions returns per-issue list
- GET /api/workflows/:id/regressions returns per-issue list with run context
- Runs with no regressions return { count: 0, issues: [] }
- Works for completed, failed, and cancelled runs
...
def _make_mock_run(
    run_id: str = "run_regression_endpoint",
    *,
    workflow_id: str = "wf_regression_endpoint",
...
class TestRunResponseRegressionCount:
    ...
    def test_regression_count_is_optional_and_nullable(self):
        ...
        # Should accept None
        resp = RunResponse(
            id="run_regression_nullable",
            workflow_id="wf_regression_endpoint",
            workflow_name="Test",
            status="completed",
            started_at=None,
            completed_at=None,
    ...
    def test_single_run_response_includes_regression_count(self):
        ...
        try:
            response = client.get("/api/runs/run_regression_endpoint")
            assert response.status_code == 200
            data = response.json()
...
# ===========================================================================
# GET /api/runs/:id/regressions endpoint
# ===========================================================================
...
class TestRunRegressionsEndpoint:
    ...
    def test_returns_200_with_regression_data(self):
        ...
        response = client.get("/api/runs/run_regression_empty/regressions")
        assert response.status_code == 200
        data = response.json()
        assert data["count"] == 1
...
# ===========================================================================
# GET /api/workflows/:id/regressions endpoint
# ===========================================================================
...
class TestWorkflowRegressionsEndpoint:
    ...
    def test_returns_200_with_regression_data(self):
        ...
        mock_service.get_workflow_regressions.return_value = {
            ...
            "issues": [
                _make_regression_issue(
                    ...
                    node_name="Analyze",
                    regression_type="assertion_regression",
                    run_id="run_regression_completed",
                    run_number=2,
                ),
                _make_regression_issue(
                    node_id="summarize",
        ...
        response = client.get("/api/workflows/wf_regression_endpoint/regressions")
        assert response.status_code == 200
        data = response.json()
        assert data["count"] == 2
    ...
    def test_issues_include_run_id_and_run_number(self):
        ...
        mock_service.get_workflow_regressions.return_value = {
            ...
            "issues": [
                _make_regression_issue(
                    ...
                    run_id="run_regression_cancelled",
                    run_number=5,
                )
            ],
        }
        ...
        assert issue["run_id"] == "run_regression_cancelled"
...
# ===========================================================================
# Completed, failed, and cancelled runs expose regression summaries
# ===========================================================================
