...
class TestEvalPassRateComputed:
    ...
    def test_all_nodes_passed_returns_1_0(self):
        ...
        now = time.time()
        run = _make_mock_run("run_eval_kpi_current", RunStatus.completed, 0.5, now - 3600)

        nodes = [
            _make_mock_node(
                "node_eval_primary",
                "run_eval_kpi_current",
                eval_passed=True,
                eval_score=0.95,
                created_at=now - 3600,
            ),
    ...
    def test_aggregates_across_multiple_recent_runs(self):
        ...
        now = time.time()
        run_eval_kpi_current = _make_mock_run(
            "run_eval_kpi_current", RunStatus.completed, 0.5, now - 3600
        )
        run_eval_kpi_secondary = _make_mock_run(
            "run_eval_kpi_secondary", RunStatus.completed, 0.3, now - 7200
        )

        # run_eval_kpi_current: 2 passed, run_eval_kpi_secondary: 1 failed => 2/3 = 0.6667
        nodes_run_eval_kpi_current = [
            _make_mock_node(
                "node_eval_primary",
                "run_eval_kpi_current",
                eval_passed=True,
        ...
        nodes_run_eval_kpi_secondary = [
            _make_mock_node(
                "node_eval_tertiary",
                "run_eval_kpi_secondary",
                eval_passed=False,
                eval_score=0.3,
                created_at=now - 7200,
            ),
        ...
        mock_service = Mock()
        mock_service.list_runs.return_value = [run_eval_kpi_current, run_eval_kpi_secondary]
        mock_service.get_run_nodes.side_effect = lambda rid: (
            nodes_run_eval_kpi_current
            if rid == "run_eval_kpi_current"
            else nodes_run_eval_kpi_secondary
        )
        app.dependency_overrides[get_run_service] = lambda: mock_service
...
class TestRegressionsComputed:
    ...
    def test_regression_detected_when_node_fails_with_passing_baseline(self):
        ...
        now = time.time()
        previous_run = _make_mock_run("run_eval_kpi_baseline", RunStatus.completed, 0.4, now - 7200)
        run = _make_mock_run("run_eval_kpi_current", RunStatus.completed, 0.5, now - 3600)

        previous_node = _make_mock_node(
            "node_eval_primary",
            "run_eval_kpi_baseline",
            eval_passed=True,
            eval_score=0.9,
            soul_id="soul_a",
            soul_version="v1",
        ...
        mock_service.get_run_nodes.side_effect = lambda rid: (
            [previous_node] if rid == "run_eval_kpi_baseline" else [node]
        )
        app.dependency_overrides[get_run_service] = lambda: mock_service
    ...
    def test_multiple_regressions_counted(self):
        ...
        mock_service.list_runs.return_value = [run, previous_run]
        mock_service.get_run_nodes.side_effect = lambda rid: (
            previous_nodes if rid == "run_eval_kpi_baseline" else nodes
        )
        app.dependency_overrides[get_run_service] = lambda: mock_service
