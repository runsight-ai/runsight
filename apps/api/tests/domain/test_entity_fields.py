"""
RunNode.exit_handle field and SQLite backfill column coverage.
Run.warnings_json backfill coverage and migration.

These tests verify:
  - RunNode exposes and persists ``exit_handle``.
  - SQLite backfill covers nested-run columns on ``run``.
  - SQLite backfill covers child run columns on ``runnode``.
  - SQLite backfill and Alembic migrations cover Run.warnings_json.
"""
...
class TestRunNodeExitHandle:
    ...
    def test_run_node_exit_handle_persists_round_trip(self, tmp_path) -> None:
        ...
        db_path = tmp_path / "test_entity_fields.db"
...
class TestSqliteBackfillColumns:
    ...
    def test_ensure_sqlite_columns_backfills_warnings_json_on_legacy_run_table(
        ...
        db_path = tmp_path / "entity_fields_legacy.sqlite"
        engine = create_engine(f"sqlite:///{db_path}")
