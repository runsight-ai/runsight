"""RUN-958 regression coverage for execution transport seams.
...
def _write_secrets_file(base_dir: Path) -> None:
    ...
    (secrets_dir / "secrets.env").write_text(
        "# Managed by Runsight\nOPENAI_API_KEY=sk-fake-test-key-for-run958\n",
        encoding="utf-8",
    )
...
@pytest.fixture
def db_engine():
    db_path = Path(tempfile.mkdtemp(prefix="run958-db-")) / "runsight.db"
...
@pytest.fixture
def base_dir():
    with tempfile.TemporaryDirectory(prefix="run958-base-") as tmpdir:
        base = Path(tmpdir)
