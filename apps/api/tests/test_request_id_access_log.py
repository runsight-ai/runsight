...
# ---------------------------------------------------------------------------
# Tests — main.py registers both middleware
# ---------------------------------------------------------------------------


class TestMainRegistersMiddleware:
    """main.py create_app() must register RequestIdMiddleware and AccessLogMiddleware."""

    def test_main_imports_request_id_middleware(self):
        import inspect
        from runsight_api import main

        source = inspect.getsource(main)
        assert "RequestIdMiddleware" in source, "main.py must import RequestIdMiddleware"

    def test_main_imports_access_log_middleware(self):
        import inspect
        from runsight_api import main

        source = inspect.getsource(main)
        assert "AccessLogMiddleware" in source, "main.py must import AccessLogMiddleware"

    def test_main_registers_request_id_middleware(self):
        import inspect
        from runsight_api import main

        source = inspect.getsource(main)
        assert (
            "add_middleware(RequestIdMiddleware" in source
            or "add_middleware(\n" in source
            and "RequestIdMiddleware" in source
        ), "main.py must call add_middleware(RequestIdMiddleware)"

    def test_main_registers_access_log_middleware(self):
        import inspect
        from runsight_api import main

        source = inspect.getsource(main)
        assert (
            "add_middleware(AccessLogMiddleware" in source
            or "add_middleware(\n" in source
            and "AccessLogMiddleware" in source
        ), "main.py must call add_middleware(AccessLogMiddleware)"
