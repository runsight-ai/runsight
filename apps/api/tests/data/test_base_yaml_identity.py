"""Embedded identity must drive YAML repository lookups."""
...
def test_list_all_does_not_synthesize_missing_id_from_filename_stem(tmp_path) -> None:
    tmpdir = str(tmp_path)
...
def test_get_by_id_does_not_resolve_embedded_id_in_other_yaml_file(tmp_path) -> None:
    tmpdir = str(tmp_path)
    repo = DummyRepository(base_path=tmpdir)
...
def test_update_does_not_retarget_embedded_id_match_in_other_yaml_file(tmp_path) -> None:
    tmpdir = str(tmp_path)
    repo = DummyRepository(base_path=tmpdir)
    _write_yaml(repo.entity_dir / "legacy-name.yaml", {"id": "real-id", "name": "Legacy"})
...
def test_soul_repository_does_not_resolve_embedded_id_from_filename_fallback(tmp_path) -> None:
    tmpdir = str(tmp_path)
...
def test_soul_repository_update_does_not_retarget_embedded_id_match(tmp_path) -> None:
    tmpdir = str(tmp_path)
    repo = SoulRepository(base_path=tmpdir)
    _write_yaml(
        repo.entity_dir / "gate_evaluator.yaml",
