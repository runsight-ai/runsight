...
def task_repo(tmp_path):
    return TaskRepository(base_path=str(tmp_path))
...
class TestNormalIdsAccepted:
    ...
    @pytest.mark.parametrize("safe_id", SAFE_IDS)
    def test_task_repo_accepts_normal_ids(self, task_repo, safe_id):
        """TaskRepository._get_path should not raise for safe IDs."""
        path = task_repo._get_path(safe_id)
        assert path.name == f"{safe_id}.yaml"
...
class TestTaskRepoRejectsTraversal:
    """TaskRepository._get_path must raise ValueError for path-traversal IDs."""

    @pytest.mark.parametrize("malicious_id", DOTDOT_IDS, ids=lambda x: f"dotdot:{x}")
    def test_rejects_dotdot_traversal(self, task_repo, malicious_id):
        with pytest.raises(ValueError, match="[Pp]ath|[Ii]nvalid|[Tt]raversal"):
            task_repo._get_path(malicious_id)

    @pytest.mark.parametrize("malicious_id", SEPARATOR_IDS, ids=lambda x: f"sep:{x}")
    def test_rejects_path_separators(self, task_repo, malicious_id):
        with pytest.raises(ValueError, match="[Pp]ath|[Ii]nvalid|[Tt]raversal"):
            task_repo._get_path(malicious_id)

    @pytest.mark.parametrize("malicious_id", ABSOLUTE_IDS, ids=lambda x: f"abs:{x}")
    def test_rejects_absolute_paths(self, task_repo, malicious_id):
        with pytest.raises(ValueError, match="[Pp]ath|[Ii]nvalid|[Tt]raversal"):
            task_repo._get_path(malicious_id)

    @pytest.mark.parametrize("malicious_id", URL_ENCODED_IDS, ids=lambda x: f"url:{x}")
    def test_rejects_url_encoded_traversal(self, task_repo, malicious_id):
        with pytest.raises(ValueError, match="[Pp]ath|[Ii]nvalid|[Tt]raversal"):
            task_repo._get_path(malicious_id)
...
class TestPublicApiRejectsTraversal:
    ...
    def test_task_get_by_id_rejects_traversal(self, task_repo):
        with pytest.raises(ValueError):
            task_repo.get_by_id("../../../etc/passwd")

    def test_task_create_rejects_traversal(self, task_repo):
        with pytest.raises(ValueError):
            task_repo.create({"id": "../../../evil", "name": "Evil"})

    def test_task_delete_rejects_traversal(self, task_repo):
        with pytest.raises(ValueError):
            task_repo.delete("../../../evil")
