...
def _write_workflow_file(base_dir: Path, fixture_name: str) -> str:
...
class TestParserSoulFieldForwarding:
    def test_parse_workflow_yaml_forwards_extended_soul_fields(self, tmp_path: Path):
        _write_soul_file(
            tmp_path,
            "researcher",
            """\
        ...
        path = _write_workflow_file(tmp_path, "parser-soul-field-forwarding-extended.yaml")
    ...
    def test_parse_workflow_yaml_keeps_new_fields_optional(self, tmp_path: Path):
        _write_soul_file(
            tmp_path,
            "reviewer",
            """\
        ...
        path = _write_workflow_file(tmp_path, "parser-soul-field-forwarding-minimal.yaml")
        workflow = parse_workflow_yaml(path)
        soul = workflow.blocks["step1"].soul
