                ...
                "results": {
                    **state.results,
                    self.block_id: BlockResult(
                        output=f"{self.block_id}_round_{len(calls)}", exit_handle=handle
                    ),
                },
                "shared_memory": {
                    **state.shared_memory,
    ...
    async def execute(self, state: WorkflowState, **kwargs) -> WorkflowState:
        ...
        return state.model_copy(
            update={
                "results": {
                    **state.results,
                    self.block_id: BlockResult(output=f"{self.block_id}_ok"),
                }
            }
        )
...
async def test_loopblock_direct_execute_without_ctx_stays_backward_compatible():
    """Direct LoopBlock.execute(..., blocks=...) calls should keep the legacy path working."""
    ...
    result_state = await loop.execute(WorkflowState(), blocks=blocks)
