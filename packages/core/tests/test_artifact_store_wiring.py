...
class TestWorkflowStateArtifactStoreField:
    ...
    def test_artifact_store_accepts_in_memory_store(self):
        ...
        store = InMemoryArtifactStore(run_id="workflow-state-artifact-store-run")
    ...
    def test_artifact_store_accessible_on_instance(self):
        ...
        store = InMemoryArtifactStore(run_id="workflow-state-artifact-store-run")
        state = WorkflowState(artifact_store=store)
        assert state.artifact_store.run_id == "workflow-state-artifact-store-run"
    ...
    def test_model_copy_update_other_fields_keeps_store(self):
        ...
        copied = state.model_copy(
            update={"results": {"artifact_capture_step": "output"}, "total_cost_usd": 2.0}
        )
        assert copied.artifact_store is store
...
class TestWorkflowBlockArtifactStorePropagation:
    ...
    def test_child_state_has_parent_artifact_store(self):
        ...
        store = InMemoryArtifactStore(run_id="workflow-block-artifact-source-execution")
        parent_state = WorkflowState(
            artifact_store=store,
            shared_memory={"topic": "AI"},
        )

        child_workflow = Workflow(name="artifact_child_workflow")
        block = WorkflowBlock(
            block_id="artifact_child_workflow_block",
    ...
    async def test_child_workflow_receives_artifact_store_during_execution(self):
        ...
        mock_artifact_child_workflow = AsyncMock()
        mock_artifact_child_workflow.name = "artifact_child_workflow"
        mock_artifact_child_workflow.identity = None
        mock_artifact_child_workflow.input_schema = None
        child_final = WorkflowState(total_cost_usd=0.0, total_tokens=0)
        mock_artifact_child_workflow.run = AsyncMock(return_value=child_final)

        block = WorkflowBlock(
            block_id="artifact_child_workflow_block",
            child_workflow=mock_artifact_child_workflow,
            inputs={"topic": "shared_memory.topic"},
            outputs={},
        )
        ...
        # Verify the child state passed to child workflow run had the artifact_store
        call_args = mock_artifact_child_workflow.run.call_args
...
class TestLoopBlockArtifactStoreSharing:
    ...
    async def test_loop_block_rounds_share_same_store(self):
        ...
        store = InMemoryArtifactStore(run_id="loop-artifact-store-run")
...
class TestDispatchBlockArtifactStoreSharing:
    ...
    async def test_dispatch_state_preserves_artifact_store(self):
        ...
        store = InMemoryArtifactStore(run_id="dispatch-artifact-store-run")
...
class TestWorkflowRunArtifactStorePropagation:
    ...
    async def test_workflow_run_preserves_artifact_store(self):
        ...
        store = InMemoryArtifactStore(run_id="workflow-run-artifact-store-run")
        capturer = ArtifactCapturingBlock("artifact_capture_step")

        wf = Workflow(name="artifact_capture_workflow")
        wf.add_block(capturer)
        wf.set_entry("artifact_capture_step")
        wf.add_transition("artifact_capture_step", None)
        ...
        assert captures["artifact_capture_step"] == id(store)
        assert result.artifact_store is store
    ...
    async def test_workflow_run_multi_block_same_store(self):
        ...
        store = InMemoryArtifactStore(run_id="workflow-run-artifact-store-run")
        first_capture = ArtifactCapturingBlock("first_artifact_capture_step")
        second_capture = ArtifactCapturingBlock("second_artifact_capture_step")

        wf = Workflow(name="multi_block_artifact_capture_workflow")
        wf.add_block(first_capture)
        wf.add_block(second_capture)
        wf.set_entry("first_artifact_capture_step")
        wf.add_transition("first_artifact_capture_step", "second_artifact_capture_step")
        wf.add_transition("second_artifact_capture_step", None)

        state = WorkflowState(artifact_store=store)
        result = await wf.run(state)

        captures = result.shared_memory["artifact_store_captures"]
        assert captures["first_artifact_capture_step"] == id(store)
        assert captures["second_artifact_capture_step"] == id(store)
    ...
    async def test_workflow_run_without_artifact_store_works(self):
        ...
        capturer = ArtifactCapturingBlock("no_store_capture_step")
        wf = Workflow(name="no_artifact_store_workflow")
        wf.add_block(capturer)
        wf.set_entry("no_store_capture_step")
        wf.add_transition("no_store_capture_step", None)

        state = WorkflowState()  # No artifact_store
        result = await wf.run(state)

        captures = result.shared_memory["artifact_store_captures"]
        assert captures["no_store_capture_step"] is None
