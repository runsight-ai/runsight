...
class TestInlineSoulNoXfailMarker:
    """test_parser_soul_field_forwarding.py must not have a module-level xfail marker."""
...
class TestInlineSoulFixturesUseLibrarySouls:
    ...
    def test_yaml_fixtures_have_no_inline_soul_definitions(self):
        ...
        yaml_dicts = _extract_yaml_constants(_PARSER_SOUL_FIELD_FORWARDING_TEST_PATH)
    ...
    def test_inline_soul_tests_pass_without_xfail(self):
        ...
        result = subprocess.run(
            [
                ...
                "-m",
                "pytest",
                str(_PARSER_SOUL_FIELD_FORWARDING_TEST_PATH),
                "-v",
                "--tb=short",
                "--no-header",
                "-q",
...
class TestAssertionConfigFixturesUseLibrarySouls:
    """test_wire_assertion_configs.py must use soul_ref without inline souls."""

    def test_yaml_fixtures_have_no_inline_soul_definitions(self):
        """YAML constants must not define souls inline in a top-level souls: block."""
        yaml_dicts = _extract_yaml_constants(_WIRE_ASSERTION_CONFIGS_TEST_PATH)
        offending = _yaml_fixtures_have_inline_souls(yaml_dicts)
        assert len(offending) == 0, (
    ...
    def test_assertion_config_tests_pass_after_migration(self):
        ...
        result = subprocess.run(
            [
                ...
                "-m",
                "pytest",
                str(_WIRE_ASSERTION_CONFIGS_TEST_PATH),
                "-v",
                "--tb=short",
                "--no-header",
                "-q",
