"""SynthesizeBlock BlockContext/BlockOutput migration coverage.

Boundary: SynthesizeBlock execution and workflow dispatch must use the
BlockContext to BlockOutput contract while preserving synthesized context
formatting, missing-input failures, and execute_block WorkflowState mapping.
Owner: packages/core runtime block execution. Exit criteria: remove this
migration guard once the block execution contract is no longer in transition
and equivalent behavior coverage lives in ordinary block suites.
"""
...
# SynthesizeBlock BlockContext execution


@pytest.mark.asyncio
async def test_synthesizeblock_execute_accepts_block_context(mock_runner, synthesis_soul):
    ...
    combined = "=== Output from research_report ===\nOutput A\n\n=== Output from market_notes ===\nOutput B"
    ctx = _make_synth_block_context("final_synthesis", synthesis_soul, combined)
...
@pytest.mark.asyncio
async def test_synthesizeblock_execute_populates_cost_and_tokens(mock_runner, synthesis_soul):
    ...
    block = SynthesizeBlock("final_synthesis", ["research_report"], synthesis_soul, mock_runner)
    combined = "=== Output from research_report ===\nOutput A"
    ctx = _make_synth_block_context("final_synthesis", synthesis_soul, combined)
...
@pytest.mark.asyncio
async def test_synthesizeblock_execute_log_entries_contain_block_id(mock_runner, synthesis_soul):
    ...
    assert any("final_synthesis" in entry.get("content", "") for entry in result.log_entries), (
        "Expected log_entries to contain an entry referencing block_id 'final_synthesis'"
    )


@pytest.mark.asyncio
async def test_synthesizeblock_execute_returns_data_not_state(mock_runner, synthesis_soul):
...
# Combined output context formatting


def test_build_block_context_synthesize_two_inputs_format(mock_runner, synthesis_soul):
    ...
    state = WorkflowState(
        results={
            "research_report": BlockResult(output="Output A"),
            "market_notes": BlockResult(output="Output B"),
        }
    )
    ...
    expected = "=== Output from research_report ===\nOutput A\n\n=== Output from market_notes ===\nOutput B"
    assert ctx.context == expected, (
        f"Combined outputs format mismatch.\nExpected:\n{expected!r}\nGot:\n{ctx.context!r}"
    )


def test_build_block_context_synthesize_three_inputs_format(mock_runner, synthesis_soul):
    """build_block_context for SynthesizeBlock with 3 inputs must include all three
    in order, separated by '\\n\\n'.
    """
    block = SynthesizeBlock(
        "final_synthesis",
        ["research_report", "market_notes", "risk_summary"],
        synthesis_soul,
        mock_runner,
    )
    state = WorkflowState(
        results={
            "research_report": BlockResult(output="Output A"),
            "market_notes": BlockResult(output="Output B"),
            "risk_summary": BlockResult(output="Output C"),
        }
    )
    ...
    assert isinstance(ctx, BlockContext)
    expected = (
        "=== Output from research_report ===\nOutput A"
        "\n\n"
        "=== Output from market_notes ===\nOutput B"
        "\n\n"
        "=== Output from risk_summary ===\nOutput C"
    )
    assert ctx.context == expected, (
...
def test_build_block_context_synthesize_instruction_is_standard(mock_runner, synthesis_soul):
    """build_block_context for SynthesizeBlock must set the standard synthesis instruction."""
    block = SynthesizeBlock("final_synthesis", ["research_report"], synthesis_soul, mock_runner)
    state = WorkflowState(results={"research_report": BlockResult(output="Output A")})
...
def test_build_block_context_synthesize_populates_block_id(mock_runner, synthesis_soul):
    ...
    assert ctx.block_id == "final_synthesis", (
        f"Expected block_id='final_synthesis', got {ctx.block_id!r}"
    )


def test_build_block_context_synthesize_populates_soul(mock_runner, synthesis_soul):
    ...
    assert ctx.soul == synthesis_soul, (
        f"Expected ctx.soul to be the synthesizer soul, got {ctx.soul!r}"
    )


def test_build_block_context_synthesize_raw_string_result(mock_runner, synthesis_soul):
    """build_block_context handles BlockResult in state.results for SynthesizeBlock."""
    block = SynthesizeBlock("final_synthesis", ["research_report"], synthesis_soul, mock_runner)
    state = WorkflowState(results={"research_report": BlockResult(output="Raw string output")})
...
def test_build_block_context_synthesize_missing_one_input_raises(mock_runner, synthesis_soul):
    """build_block_context must raise ValueError when one of the input_block_ids is absent."""
    block = SynthesizeBlock(
        "final_synthesis", ["research_report", "market_notes"], synthesis_soul, mock_runner
    )
    state = WorkflowState(
        results={"research_report": BlockResult(output="Output A")},
        # market_notes deliberately missing
    )
    ...
    assert "market_notes" in error_msg, (
        f"ValueError must mention the missing block id 'market_notes'. Got: {error_msg!r}"
    )


def test_build_block_context_synthesize_missing_all_inputs_raises(mock_runner, synthesis_soul):
    ...
    with pytest.raises(ValueError) as exc_info:
        build_block_context(block, state)
    assert "research_report" in str(exc_info.value) or "market_notes" in str(exc_info.value), (
        f"ValueError must mention at least one missing block id ('research_report' or 'market_notes'). "
        f"Got: {str(exc_info.value)!r}"
    )


def test_build_block_context_synthesize_error_message_lists_available(mock_runner, synthesis_soul):
    ...
    state = WorkflowState(
        results={
            "research_report": BlockResult(output="Output A"),
            "other_block": BlockResult(output="X"),
        },
        # market_notes missing; other_block is present
    )
    ...
    assert "market_notes" in error_msg, (
        f"Expected 'market_notes' in error message. Got: {error_msg!r}"
    )
    # Helpful: should also reference available keys
    assert (
        "research_report" in error_msg or "other_block" in error_msg or "Available" in error_msg
...
async def test_execute_block_dispatches_synthesizeblock_via_new_path(
    ...
    assert "final_synthesis" in result_state.results
...
async def test_execute_block_synthesizeblock_state_has_output(
    ...
    assert isinstance(result_state, WorkflowState)
    assert "final_synthesis" in result_state.results
    assert isinstance(result_state.results["final_synthesis"], BlockResult)
    assert result_state.results["final_synthesis"].output == "Cohesive final synthesis."
...
async def test_execute_block_synthesizeblock_combined_context_passed_to_llm(
    ...
    assert "=== Output from research_report ===" in context_arg
    assert "=== Output from market_notes ===" in context_arg
...
async def test_execute_block_synthesizeblock_execution_log_extended(
    ...
    assert any("final_synthesis" in e.get("content", "") for e in result_state.execution_log[1:]), (
        "New log entry must reference block_id 'final_synthesis'"
    )
...
async def test_execute_block_synthesizeblock_missing_input_raises_before_llm_call(
    ...
    state = WorkflowState(
        results={"research_report": BlockResult(output="Output A")}
        # market_notes missing
    )

    with pytest.raises(ValueError) as exc_info:
        await execute_block(block, state, block_execution_ctx)

    assert "market_notes" in str(exc_info.value)
...
async def test_execute_block_preserves_prior_results_after_synthesize(
    ...
    assert "research_report" in result_state.results
    assert result_state.results["research_report"].output == "Output A"
    assert "market_notes" in result_state.results
    assert result_state.results["market_notes"].output == "Output B"
    # And the new synth result is added
    assert "final_synthesis" in result_state.results
    assert result_state.results["final_synthesis"].output == "Synthesized."
