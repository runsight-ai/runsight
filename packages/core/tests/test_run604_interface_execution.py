"""RUN-604 expectations after RUN-922 removes legacy workflow interfaces."""
...
class _CapturingWorkflow:
    def __init__(self, name: str = "child_wf") -> None:
        self.name = name
...
class TestNameBasedWorkflowInvocation:
    async def test_workflow_block_passes_public_input_names_via_child_inputs_kwarg(self) -> None:
        ...
        assert child_wf.received_kwargs is not None
        assert child_wf.received_kwargs["inputs"] == {"query": "climate change"}
        assert child_wf.received_state is not None
        assert child_wf.received_state.results == {}
        assert child_wf.received_state.shared_memory == {}
        assert child_wf.received_state.metadata == {}
        assert result_state.results["invoke_child"].exit_handle == "completed"
    ...
    async def test_workflow_block_unwraps_parent_block_results_for_invocation_inputs(self) -> None:
        ...
        assert child_wf.received_kwargs is not None
        assert child_wf.received_kwargs["inputs"] == {"query": "from result"}
    ...
    async def test_workflow_block_rejects_private_child_state_input_targets(self) -> None:
        ...
        assert child_wf.received_state is None
