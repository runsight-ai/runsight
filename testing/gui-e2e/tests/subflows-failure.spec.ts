...
const TEST_PREFIX = `e2e-subflows-failing-${Date.now()}`;

test("subflows report failed child execution from the editor", async ({ page }) => {
  ...
  try {
    ...
    const child = await createWorkflowViaUi(
      page,
      childName,
      (workflowId) => buildFailingChildYaml(workflowId, childName),
    ...
    const parent = await createWorkflowViaUi(
      page,
      parentName,
      (workflowId) => buildFailingParentYaml(workflowId, parentName, child.id),
      (workflow) => createdWorkflows.push(workflow),
    );
    ...
    const parentRun = await waitForWorkflowRun(parent.id, "failed");
    createdRunIds.push(parentRun.id);
    const childRun = await waitForChildRun(parentRun.id, "failed");
    createdRunIds.push(childRun.id);
    const subflowNode = await waitForRunNode(parentRun.id, "call_failing_child", "failed");

    expect(subflowNode.status).toBe("failed");
