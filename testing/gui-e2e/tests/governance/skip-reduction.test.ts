/**
 * Governance coverage for Playwright skip-driven coverage loss.
 *
 * Source-reading checks that assert the Playwright spec files meet the
 * skip-reduction targets owned by the E2E workspace:
 *
 * - total test.skip() sites <= 31
 * - no files use "previous test" cascade skip messages
 * - key files use beforeAll for setup, not previous-test state
 * - conditional variable-cascade skips are eliminated
 *
 * Exit criteria: delete this governance suite once the same constraints are
 * enforced by fixture helpers or lint rules inside testing/gui-e2e.
 */
...
// ---------------------------------------------------------------------------
// Total test.skip() count across all spec files must be <= 31.
//
// ---------------------------------------------------------------------------

describe("Playwright skip count governance", () => {
...
// ---------------------------------------------------------------------------
// No files should use the "previous test" cascade skip message pattern.
//
...
describe("Playwright cascade skip message governance", () => {
...
// ---------------------------------------------------------------------------
// workflows-crud.spec.ts must use beforeAll for setup.
//
...
describe("Playwright setup fixture governance", () => {
...
// ---------------------------------------------------------------------------
// CRUD files that still lack beforeAll must be identified.
//
...
describe("Playwright CRUD setup governance", () => {
...
// ---------------------------------------------------------------------------
// Variable-cascade skips, such as test.skip(!createdId), must be eliminated.
//
...
describe("Playwright variable-cascade skip governance", () => {
