...
export const WORKFLOW_ID = "wf_context_governance";
export const COMPLETED_RUN_ID = "run_context_governance_completed";
export const LIVE_RUN_ID = "run_context_governance_live";
export const LONG_REF =
...
function run(id: string, status: "completed" | "running" | "failed") {
  return {
    ...
    workflow_name: "Context Governance",
    status,
    ...
    branch: "main",
    source: "manual",
    commit_sha: "abc-context-governance",
    run_number: status === "completed" ? 1 : 2,
    regression_count: 0,
  };
}
...
export async function installContextAuditRoutes(
  page: Page,
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (request.method() === "POST" && path === "/api/workflows") {
      ...
      return json(route, {
        ...workflow(),
        id: "drft-context-governance-a1b2",
        name: "Context Governance fork",
        yaml: String(body.yaml ?? ""),
      });
    }
    if (path === "/api/workflows/drft-context-governance-a1b2") {
      return json(route, {
        ...workflow(),
        id: "drft-context-governance-a1b2",
        name: "Context Governance fork",
