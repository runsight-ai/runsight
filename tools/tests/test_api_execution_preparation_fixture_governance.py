"""Governance tests for API execution preparation workflow fixture ownership.

Owner: tools/tests owns temporary static checks for API test fixture ownership
migrations.
Boundary: reusable workflow payloads for the API execution preparation suite
belong under apps/api/tests/fixtures and must not remain as module-level inline
YAML constants in apps/api/tests/logic/test_execution_preparation.py. Inline
YAML returned from local builder functions for per-test snapshot edge cases is
intentionally outside this static check.
Exit criteria: delete this suite once reusable execution preparation workflow
payloads have been externalized and the API package suite has stable
fixture-loading coverage for execution preparation.
"""
...
def test_api_execution_preparation_suite_uses_package_owned_workflow_fixtures() -> None:
    """Reusable API execution preparation workflows should live in API-owned fixtures."""
    constants = _module_level_workflow_constants(EXECUTION_PREPARATION_TEST)

    assert constants == [], (
        f"{_relative(EXECUTION_PREPARATION_TEST)} must not define reusable "
        "module-level workflow YAML constants. Move reusable execution preparation "
