"""Governance tests for API run creation execution fixture ownership.

Owner: tools/tests owns temporary static checks for API test fixture ownership
migrations.
Boundary: reusable workflow payloads for the API run creation execution suite
...
Exit criteria: delete this suite once reusable run creation execution workflow
...
RUN_CREATION_EXECUTION_TEST = (
    REPO_ROOT / "apps" / "api" / "tests" / "test_run_creation_execution_integration.py"
)
...
def test_api_run_creation_execution_suite_uses_package_owned_workflow_fixtures() -> None:
    """Reusable API run creation execution workflows should live in API-owned fixtures."""
    constants = _module_level_workflow_constants(RUN_CREATION_EXECUTION_TEST)

    assert constants == [], (
        f"{_relative(RUN_CREATION_EXECUTION_TEST)} must not define reusable "
        "module-level workflow YAML constants. Move reusable run creation execution "
        f"workflow payloads to {_relative(API_FIXTURE_ROOT)} and load them from the "
        "API package tests. This check is intentionally scoped to module-level "
        "workflow documents, so tiny inline request payloads inside individual "
        "tests remain outside the boundary. Found:\n"
        + "\n".join(
...
def test_api_run_creation_execution_runtime_workspace_fixture_uses_tmp_path() -> None:
    """Owner/boundary/exit: base_dir runtime workspace must use pytest tmp_path."""
    usages = _runtime_workspace_tempfile_usages(RUN_CREATION_EXECUTION_TEST)

    assert usages == [], (
        f"{_relative(RUN_CREATION_EXECUTION_TEST)} {RUNTIME_WORKSPACE_FIXTURE} fixture "
        ...
        "base_dir, so db_engine in-memory setup and tiny inline request payloads "
        "remain outside the boundary. Found:\n"
        + "\n".join(
