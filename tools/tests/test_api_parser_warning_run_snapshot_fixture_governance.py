"""Governance tests for API parser warning run snapshot fixture ownership.

Owner: tools/tests owns temporary static checks for API test fixture ownership
migrations.
Boundary: reusable workflow payloads for parser warning run snapshot coverage
...
Exit criteria: delete this suite once reusable parser warning run snapshot
...
PARSER_WARNING_RUN_SNAPSHOT_TEST = (
    REPO_ROOT / "apps" / "api" / "tests" / "test_parser_warning_run_snapshots.py"
)
...
def test_api_parser_warning_run_snapshot_suite_uses_api_owned_workflow_fixtures() -> None:
    """Reusable parser warning run snapshot workflows should live in API fixtures."""
    constants = _module_level_workflow_constants(PARSER_WARNING_RUN_SNAPSHOT_TEST)

    assert constants == [], (
        f"{_relative(PARSER_WARNING_RUN_SNAPSHOT_TEST)} must not define reusable "
        "module-level workflow YAML constants. Move reusable parser warning run "
        f"snapshot workflow payloads to {_relative(API_FIXTURE_ROOT)} and load them "
        "from the API package tests. This check only flags module-level workflow "
        "documents, so the parameterized _warning_workflow_yaml builder, helper "
        "writer YAML fragments, db_engine in-memory setup, and tiny request "
...
def test_api_parser_warning_run_snapshot_runtime_workspace_fixture_uses_tmp_path() -> None:
    """Owner/boundary/exit: base_dir runtime workspace must use pytest tmp_path."""
    fixture = _runtime_workspace_fixture(PARSER_WARNING_RUN_SNAPSHOT_TEST)
    fixture_args = _fixture_argument_names(fixture)
    usages = _runtime_workspace_tempfile_usages(PARSER_WARNING_RUN_SNAPSHOT_TEST)

    assert fixture is not None, (
        f"{_relative(PARSER_WARNING_RUN_SNAPSHOT_TEST)} must keep a pytest "
        f"{RUNTIME_WORKSPACE_FIXTURE} fixture for runtime workspace setup."
    )
    assert "tmp_path" in fixture_args and usages == [], (
        f"{_relative(PARSER_WARNING_RUN_SNAPSHOT_TEST)} {RUNTIME_WORKSPACE_FIXTURE} fixture "
        ...
        "in-memory setup, the parameterized _warning_workflow_yaml builder, helper "
        "writer YAML fragments, and tiny request payloads remain outside the "
        "boundary. Found:\n"
