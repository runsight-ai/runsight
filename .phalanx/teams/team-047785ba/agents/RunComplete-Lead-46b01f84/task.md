...
## Your Task
Team task: Build Flow 2.07: Run Complete (Post-Execution Summary) for Phalanx GUI canvas. When a workflow run finishes, show: (1) Summary toast with duration/cost/status, (2) Nodes in final state (green Completed, red Failed — no pulse), (3) Run button changes to 'Run Again' or 'Retry', (4) Bottom panel shows 'Run complete', (5) Inspector Execution tab shows final metrics. Reference .agora/mockups/flow-2-create-run-pm/07-run-complete/. Write E2E tests. Reviewer gates.

Workers:
- RunComplete-UI-6fee8932 (role=coder, name=RunComplete-UI)
- RunComplete-E2E-e1bff6ab (role=coder, name=RunComplete-E2E)
- RunComplete-Reviewer-8ab35ed9 (role=reviewer, name=RunComplete-Reviewer)

Team ID: team-047785ba
