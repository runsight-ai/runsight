...
## Your Task
Team task: Build Flow 3.01: YAML Editor (Code Mode) for Phalanx GUI canvas. Full-screen or split-view YAML editor using Monaco (or textarea fallback). [Visual | Code] segmented control in toolbar. Sync indicator. YAML toolbar with Save/Undo/Format. Bi-directional sync with canvas. Status bar: validation (✓ Valid YAML / ✗ Error), node/edge count, Modified. Reference .agora/mockups/flow-3-create-run-engineer/01-yaml-editor/. Write E2E tests. Reviewer gates.

Workers:
- YamlEditor-UI-b508955e (role=coder, name=YamlEditor-UI)
- YamlEditor-E2E-e223e920 (role=coder, name=YamlEditor-E2E)
- YamlEditor-Reviewer-7c46b382 (role=reviewer, name=YamlEditor-Reviewer)

Team ID: team-2daa62c0
