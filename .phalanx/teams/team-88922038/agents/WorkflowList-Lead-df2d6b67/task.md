...
## Your Task
Team task: Build the Workflow List page for Phalanx GUI. Replace the stub at apps/gui/src/features/workflows/WorkflowList.tsx with a full implementation. Route: /workflows. Table of workflows with name, description, step count, last run status, cost, updated_at. New Workflow button triggers existing NewWorkflowModal. Row click navigates to /workflows/:id (canvas). Search, filter, empty state. Write E2E tests. Reviewer gates.

Workers:
- WorkflowList-UI-1ee94fb7 (role=coder, name=WorkflowList-UI)
- WorkflowList-E2E-946912fd (role=coder, name=WorkflowList-E2E)
- WorkflowList-Reviewer-5aaeb014 (role=reviewer, name=WorkflowList-Reviewer)

Team ID: team-88922038
