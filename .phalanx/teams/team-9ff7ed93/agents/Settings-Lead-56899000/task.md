...
## Your Task
Team task: Build the Settings page (Flow 5) for Phalanx GUI. Replace the current stub at apps/gui/src/features/settings/SettingsPage.tsx with a full implementation: tabbed layout with Providers, Models, API Keys, and Budgets tabs, plus an Add Provider dialog. Must use existing component library, existing Zod schemas, existing React Query hooks, and match the HTML mockups pixel-perfectly. Backend API is live at localhost:8000. Write Playwright e2e tests.

Workers:
- Settings-Coder-88e070a1 (role=coder, name=Settings-Coder)
- Settings-Reviewer-95c6b1d2 (role=reviewer, name=Settings-Reviewer)

Team ID: team-9ff7ed93
